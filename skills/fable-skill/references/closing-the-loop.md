...
Modifier on any planning invocation (`/fable-skill --issues`, `/fable-skill security --issues`). The flag is the user's authorization to create issues — never create them without it.
...
3. Per plan: `gh issue create --title "<plan title>" --body-file <plan file>`. Labels: `fable-skill` plus the category — apply only if the labels exist or can be created without erroring; skip labels rather than fail.
